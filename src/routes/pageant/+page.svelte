<section class="hero bg-section flex flex-col">
	
	<!-- <img src="/img/event-banner.webp" alt="Events" class="hidden lg:inline-block p-4" />
	<img src="/img/event-banner02.webp" alt="Events" class="lg:hidden p-4" /> -->
	<br><br><br>
	<h1 class="font-bold text-center pb-20">2023 Contestants Coming Soon!</h1>

</section>

<style>
	.bg-section {
		background: center / cover url('/isometric.svg'), linear-gradient(to bottom right, pink, purple);
		background-blend-mode: darken;
	}
</style>
